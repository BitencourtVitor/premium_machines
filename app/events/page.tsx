...
export default function EventsPage() {
  ...
  const handleEditEvent = (event: AllocationEvent) => {
    ...
    setNewEvent({
      ...
      requested_at: event.requested_at ? getLocalDateForInput(new Date(event.requested_at)) : '',
      validated_by_name: event.validated_by_name || '',
      validated_at: event.validated_at ? getLocalDateForInput(new Date(event.validated_at)) : '',
  ...
  const handleStartFromRequest = (request: AllocationEvent) => {
    ...
    setNewEvent({
      ...
      requested_at: request.requested_at ? getLocalDateForInput(new Date(request.requested_at)) : '',
      validated_by_name: request.validated_by_name || '',
      validated_at: request.validated_at ? getLocalDateForInput(new Date(request.validated_at)) : '',
